...
class FoldingTicket extends StatefulWidget {
  ...
  const FoldingTicket({this.duration, required this.entries, this.isOpen = false, this.onClick});
...
class _FoldingTicketState extends State<FoldingTicket> with SingleTickerProviderStateMixin {
    ...
    return Container(
      ...
      child: Container(
        decoration: BoxDecoration(
          boxShadow: [BoxShadow(color: context.inverseSurface!.withOpacity(.1), blurRadius: 10, spreadRadius: 1)],
        ),
        child: _buildEntry(0),
      ),
    );
  ...
  Widget _buildEntry(int index) {
    ...
    final Widget card = SizedBox(height: entry.height, child: ratio < 0.5 ? entry.back : entry.front);
